...
export function ControlPanel({ stateMachine, currentState, currentParentState = null, onTrigger, onReset }: Props) {
  ...
  return (
    <div className={styles.panel}>
      ...
      <section className={styles.section}>
        ...
        {allEmpty ? (
          ...
          <div className={styles.triggers}>
            {showHeaders && <h3 className={styles.groupHeader}>現在状態のトリガー</h3>}
            ...
            {showHeaders && <h3 className={styles.groupHeader}>共通トリガー ({currentParentState})</h3>}
            {parentTransitions.map((t, i) => (
              <button
                ...
                onClick={() => onTrigger(t)}
              >
                <span className={styles.commonLabel}>[{currentParentState}]</span> {t.trigger}
                <span className={styles.arrow}>
                  → {resolveDestination(t.to, parentStates)}
                </span>
              </button>
      ...
      <section className={styles.section}>
        <div className={styles.toggleHeader}>
          <h2 className={styles.title}>遷移定義一覧</h2>
          <button
            className={styles.toggleBtn}
